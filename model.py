...
#architecture
def model(input, is_training=True, output_channels=3):
...
class denoiser(object):
    def __init__(self, sess, sigma, ckpt_dir, sample_dir, log_dir):
        ...
        with tf.variable_scope('CDnCNN'):
            self.Y_ = model(self.Y, is_training=self.is_training)
    ...
    def train(self, data_path, eval_files, batch_size, epoch, lr):
        ...
        data = np.load('./patches/clean_pats_rgb.npy', mmap_mode='r')
        print("DATA RANGE:")
        print(np.amax(data))
        print(np.amin(data))
    ...
    def save(self, iter_num, ckpt_dir, model_name='model'):
